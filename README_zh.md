...
支持 **Ollama / WebLLM / 云端 API** 三种引擎自由切换，搭配极简**原生卡片界面**，开箱即用。无云端依赖，无隐私焦虑，让智能回归轻盈。
...
- 🤖 **三擎合一** — 自由切换 **WebLLM** (内置轻量)、**Ollama** (本地强力) 或 **云端 API** (自定连接)
- 🎭 **丰富角色库** — 内置 10+ 种精选角色提示词 (写作、翻译、润色、润色等)，支持一键切换与自定义
- 🌍 **全球沟通** — **支持 8 种语言**: 
  - 简体中文, English, 繁體中文, 日本語, 한국어, Français, Deutsch, Español
  - 界面与 AI 回复自动适配您的语言设置
- 🔒 **隐私优先** — 所有 AI 推理在本地完成 (使用本地引擎时)，无数据上传，通过 Apple 官方公证
- 💬 **高度定制** — 可自由编辑系统提示词，打造该专属 AI 助手
...
### 1. WebLLM (内置轻量)
应用内置了 WebLLM 引擎和 `qwen2.5:0.5b` 轻量模型。
- **优点**: 无需安装任何额外软件，下载应用即可直接使用。
...
- **配置**: 在设置中填入 API 地址和 Key 即可 (支持 DeepSeek, Moonshot 等)。
