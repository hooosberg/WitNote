...
function schemaRefOrVal$1({ topSchemaRef, schemaPath }, schema, keyword2, $data) {
  if (!$data) {
    ...
    if (typeof schema == "string")
      return (0, codegen_1$X._)`${schema}`;
  }
  return (0, codegen_1$X._)`${topSchemaRef}${schemaPath}${(0, codegen_1$X.getProperty)(keyword2)}`;
}
...
function makeMergeEvaluated$1({ mergeNames, mergeToName, mergeValues, resultToName }) {
  return (gen, from, to, toName) => {
    const res = to === void 0 ? from : to instanceof codegen_1$X.Name ? (from instanceof codegen_1$X.Name ? mergeNames(gen, from, to) : mergeToName(gen, from, to), to) : from instanceof codegen_1$X.Name ? (mergeToName(gen, to, from), from) : mergeValues(from, to);
    return toName === codegen_1$X.Name && !(res instanceof codegen_1$X.Name) ? resultToName(gen, res) : res;
  };
}
util$1.mergeEvaluated = {
  props: makeMergeEvaluated$1({
    mergeNames: (gen, from, to) => gen.if((0, codegen_1$X._)`${to} !== true && ${from} !== undefined`, () => {
      gen.if((0, codegen_1$X._)`${from} === true`, () => gen.assign(to, true), () => gen.assign(to, (0, codegen_1$X._)`${to} || {}`).code((0, codegen_1$X._)`Object.assign(${to}, ${from})`));
    }),
    mergeToName: (gen, from, to) => gen.if((0, codegen_1$X._)`${to} !== true`, () => {
      if (from === true) {
        gen.assign(to, true);
      } else {
        gen.assign(to, (0, codegen_1$X._)`${to} || {}`);
  ...
  items: makeMergeEvaluated$1({
    mergeNames: (gen, from, to) => gen.if((0, codegen_1$X._)`${to} !== true && ${from} !== undefined`, () => gen.assign(to, (0, codegen_1$X._)`${from} === true ? true : ${to} > ${from} ? ${to} : ${from}`)),
    mergeToName: (gen, from, to) => gen.if((0, codegen_1$X._)`${to} !== true`, () => gen.assign(to, from === true ? true : (0, codegen_1$X._)`${to} > ${from} ? ${to} : ${from}`)),
...
function evaluatedPropsToName$1(gen, ps) {
  ...
  const props = gen.var("props", (0, codegen_1$X._)`{}`);
  if (ps !== void 0)
...
function setEvaluated$1(gen, props, ps) {
  Object.keys(ps).forEach((p) => gen.assign((0, codegen_1$X._)`${props}${(0, codegen_1$X.getProperty)(p)}`, true));
}
...
function getErrorPath$1(dataProp, dataPropType, jsPropertySyntax) {
  if (dataProp instanceof codegen_1$X.Name) {
    const isNumber = dataPropType === Type$1.Num;
    return jsPropertySyntax ? isNumber ? (0, codegen_1$X._)`"[" + ${dataProp} + "]"` : (0, codegen_1$X._)`"['" + ${dataProp} + "']"` : isNumber ? (0, codegen_1$X._)`"/" + ${dataProp}` : (0, codegen_1$X._)`"/" + ${dataProp}.replace(/~/g, "~0").replace(/\\//g, "~1")`;
  }
  return jsPropertySyntax ? (0, codegen_1$X.getProperty)(dataProp).toString() : "/" + escapeJsonPointer$1(dataProp);
}
...
const names$2 = {
  ...
  data: new codegen_1$W.Name("data"),
  // data passed to validation function
  // args passed from referencing schema
  valCxt: new codegen_1$W.Name("valCxt"),
  // validation/data context - should not be used directly, it is destructured to the names below
  instancePath: new codegen_1$W.Name("instancePath"),
  parentData: new codegen_1$W.Name("parentData"),
  parentDataProperty: new codegen_1$W.Name("parentDataProperty"),
  rootData: new codegen_1$W.Name("rootData"),
  // root data - same as the data passed to the first/top validation function
  dynamicAnchors: new codegen_1$W.Name("dynamicAnchors"),
  // used to support recursiveRef and dynamicRef
  // function scoped variables
  vErrors: new codegen_1$W.Name("vErrors"),
  // null or array of validation errors
  errors: new codegen_1$W.Name("errors"),
  // counter of validation errors
  this: new codegen_1$W.Name("this"),
  // "globals"
  self: new codegen_1$W.Name("self"),
  scope: new codegen_1$W.Name("scope"),
  // JTD serialize/parse name for JSON string and position
  json: new codegen_1$W.Name("json"),
  jsonPos: new codegen_1$W.Name("jsonPos"),
  jsonLen: new codegen_1$W.Name("jsonLen"),
  jsonPart: new codegen_1$W.Name("jsonPart")
};
...
const def$M = {
  ...
  code(cxt) {
    ...
    if (opts.removeAdditional !== "all" && (0, util_1$z.alwaysValidSchema)(it, schema))
      return;
    const props = (0, code_1$c.allSchemaProperties)(parentSchema.properties);
    const patProps = (0, code_1$c.allSchemaProperties)(parentSchema.patternProperties);
    ...
    function isAdditional(key) {
      ...
      if (props.length > 8) {
        ...
        definedProp = (0, code_1$c.isOwnProperty)(gen, propsSchema, key);
      ...
      if (patProps.length) {
        definedProp = (0, codegen_1$B.or)(definedProp, ...patProps.map((p) => (0, codegen_1$B._)`${(0, code_1$c.usePattern)(cxt, p)}.test(${key})`));
      }
...
const def$K = {
  ...
  code(cxt) {
    ...
    const { opts } = it;
    const patterns = (0, code_1$9.allSchemaProperties)(schema);
    ...
    function validateProperties(pat) {
      gen.forIn("key", data, (key) => {
        gen.if((0, codegen_1$A._)`${(0, code_1$9.usePattern)(cxt, pat)}.test(${key})`, () => {
...
function hasSchema$1(it, keyword2) {
  ...
  return schema !== void 0 && !(0, util_1$s.alwaysValidSchema)(it, schema);
}
_if$1.default = def$F;
var thenElse$1 = {};
...
const codegen_1$p = codegen$1;
const error$k = {
  message: ({ schemaCode }) => (0, codegen_1$p.str)`must match format "${schemaCode}"`,
  params: ({ schemaCode }) => (0, codegen_1$p._)`{format: ${schemaCode}}`
};
const def$u = {
  keyword: "format",
  type: ["number", "string"],
  ...
  code(cxt, ruleType) {
    ...
    function validate$DataFormat() {
      ...
      const fDef = gen.const("fDef", (0, codegen_1$p._)`${fmts}[${schemaCode}]`);
      const fType = gen.let("fType");
      const format2 = gen.let("format");
      gen.if((0, codegen_1$p._)`typeof ${fDef} == "object" && !(${fDef} instanceof RegExp)`, () => gen.assign(fType, (0, codegen_1$p._)`${fDef}.type || "string"`).assign(format2, (0, codegen_1$p._)`${fDef}.validate`), () => gen.assign(fType, (0, codegen_1$p._)`"string"`).assign(format2, fDef));
      cxt.fail$data((0, codegen_1$p.or)(unknownFmt(), invalidFmt()));
      function unknownFmt() {
        if (opts.strictSchema === false)
          return codegen_1$p.nil;
        return (0, codegen_1$p._)`${schemaCode} && !${format2}`;
      }
      function invalidFmt() {
        const callFormat = schemaEnv.$async ? (0, codegen_1$p._)`(${fDef}.async ? await ${format2}(${data}) : ${format2}(${data}))` : (0, codegen_1$p._)`${format2}(${data})`;
        const validData = (0, codegen_1$p._)`(typeof ${format2} == "function" ? ${callFormat} : ${format2}.test(${data}))`;
        return (0, codegen_1$p._)`${format2} && ${format2} !== true && ${fType} === ${ruleType} && !${validData}`;
      }
    }
    function validateFormat() {
      ...
      function getFormat(fmtDef) {
        const code2 = fmtDef instanceof RegExp ? (0, codegen_1$p.regexpCode)(fmtDef) : opts.code.formats ? (0, codegen_1$p._)`${opts.code.formats}${(0, codegen_1$p.getProperty)(schema)}` : void 0;
        const fmt = gen.scopeValue("formats", { key: schema, ref: fmtDef, code: code2 });
        if (typeof fmtDef == "object" && !(fmtDef instanceof RegExp)) {
          return [fmtDef.type || "string", fmtDef.validate, (0, codegen_1$p._)`${fmt}.validate`];
        }
      ...
      function validCondition() {
        if (typeof formatDef == "object" && !(formatDef instanceof RegExp) && formatDef.async) {
          ...
          return (0, codegen_1$p._)`await ${fmtRef}(${data})`;
        }
        return typeof format2 == "function" ? (0, codegen_1$p._)`${fmtRef}(${data})` : (0, codegen_1$p._)`${fmtRef}.test(${data})`;
      }
    }
  }
};
...
const error$j = {
  ...
  params: ({ params: { discrError, tag, tagName } }) => (0, codegen_1$o._)`{error: ${discrError}, tag: ${tagName}, tagValue: ${tag}}`
};
const def$t = {
  ...
  code(cxt) {
    ...
    const valid2 = gen.let("valid", false);
    const tag = gen.const("tag", (0, codegen_1$o._)`${data}${(0, codegen_1$o.getProperty)(tagName)}`);
    gen.if((0, codegen_1$o._)`typeof ${tag} == "string"`, () => validateMapping(), () => cxt.error(false, { discrError: types_1$1.DiscrError.Tag, tag, tagName }));
    cxt.ok(valid2);
    function validateMapping() {
      const mapping = getMapping();
      gen.if(false);
      for (const tagValue in mapping) {
        gen.elseIf((0, codegen_1$o._)`${tag} === ${tagValue}`);
    ...
    function applyTagSchema(schemaProp) {
      ...
      cxt.mergeEvaluated(schCxt, codegen_1$o.Name);
      return _valid;
    }
    function getMapping() {
      var _a;
