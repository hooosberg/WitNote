...
function funcKeywordCode(cxt, def2) {
  ...
  function assignValid(_await = def2.async ? (0, codegen_1$t._)`await ` : codegen_1$t.nil) {
    ...
    gen.assign(valid2, (0, codegen_1$t._)`${_await}${(0, code_1$d.callValidateCode)(cxt, validateRef, passCxt, passSchema)}`, def2.modifying);
  }
...
function addErrs(cxt, errs) {
  ...
  gen.if((0, codegen_1$t._)`Array.isArray(${errs})`, () => {
    gen.assign(names_1$c.default.vErrors, (0, codegen_1$t._)`${names_1$c.default.vErrors} === null ? ${errs} : ${names_1$c.default.vErrors}.concat(${errs})`).assign(names_1$c.default.errors, (0, codegen_1$t._)`${names_1$c.default.vErrors}.length`);
...
function validateFunction$1({ gen, validateName, schema, schemaEnv, opts }, body) {
  if (opts.code.es5) {
    ...
    gen.func(validateName, (0, codegen_1$r._)`${names_1$6.default.data}, ${destructureValCxt$1(opts)}`, schemaEnv.$async, () => gen.code(funcSourceUrl$1(schema, opts)).code(body));
  }
}
...
function checkKeywords$1(it) {
  (0, util_1$C.checkUnknownRules)(it);
...
function checkRefsAndKeywords$1(it) {
  ...
  if (schema.$ref && opts.ignoreKeywordsWithRef && (0, util_1$C.schemaHasRulesButRef)(schema, self.RULES)) {
...
function checkNoDefault$1(it) {
  ...
  if (schema.default !== void 0 && opts.useDefaults && opts.strictSchema) {
    (0, util_1$C.checkStrictMode)(it, "default is ignored in the schema root");
  }
}
...
function returnResults$1(it) {
  ...
  if (schemaEnv.$async) {
    gen.if((0, codegen_1$r._)`${names_1$6.default.errors} === 0`, () => gen.return(names_1$6.default.data), () => gen.throw((0, codegen_1$r._)`new ${ValidationError3}(${names_1$6.default.vErrors})`));
  } else {
...
function schemaKeywords$1(it, types2, typeErrors, errsCount) {
  ...
  const { RULES } = self;
  if (schema.$ref && (opts.ignoreKeywordsWithRef || !(0, util_1$C.schemaHasRulesButRef)(schema, RULES))) {
...
function strictTypesError$1(it, msg) {
  ...
  (0, util_1$C.checkStrictMode)(it, msg, it.opts.strictTypes);
}
let KeywordCxt$1 = class KeywordCxt {
  constructor(it, def2, keyword2) {
    ...
    this.schemaValue = (0, util_1$C.schemaRefOrVal)(it, this.schema, keyword2, this.$data);
  ...
  mergeEvaluated(schemaCxt, toName) {
    ...
    if (it.props !== true && schemaCxt.props !== void 0) {
      it.props = util_1$C.mergeEvaluated.props(gen, schemaCxt.props, it.props, toName);
    }
    if (it.items !== true && schemaCxt.items !== void 0) {
      it.items = util_1$C.mergeEvaluated.items(gen, schemaCxt.items, it.items, toName);
    }
  }
...
function getData$1($data, { dataLevel, dataNames, dataPathArr }) {
  ...
  for (const segment of segments) {
    if (segment) {
      data = (0, codegen_1$r._)`${data}${(0, codegen_1$r.getProperty)((0, util_1$C.unescapeJsonPointer)(segment))}`;
...
function getJsonPointer$1(parsedRef, { baseId, schema, root }) {
  ...
  for (const part of parsedRef.fragment.slice(1).split("/")) {
    if (typeof schema === "boolean")
      return;
    const partSchema = schema[(0, util_1$q.unescapeFragment)(part)];
  ...
  let env2;
  if (typeof schema != "boolean" && schema.$ref && !(0, util_1$q.schemaHasRulesButRef)(schema, this.RULES)) {
...
const def$L = {
  ...
  code(cxt) {
    ...
    const allProps = (0, code_1$9.allSchemaProperties)(schema);
    for (const prop of allProps) {
      it.definedProperties.add(prop);
    }
    if (it.opts.unevaluated && allProps.length && it.props !== true) {
      it.props = util_1$p.mergeEvaluated.props(gen, (0, util_1$p.toHash)(allProps), it.props);
    }
    const properties2 = allProps.filter((p) => !(0, util_1$p.alwaysValidSchema)(it, schema[p]));
    ...
    for (const prop of properties2) {
      if (hasDefault(prop)) {
        ...
        gen.if((0, code_1$9.propertyInData)(gen, data, prop, it.opts.ownProperties));
        applyPropertySchema(prop);
        if (!it.allErrors)
