...
function loadMenuLanguage(lang) {
  try {
    ...
    const langFilePath = isDev ? path.join(__dirname, "../src/locales", `${lang}.json`) : path.join(process.resourcesPath, "app.asar/dist/assets/locales", `${lang}.json`);
    if (fs$1.existsSync(langFilePath)) {
      const content2 = fs$1.readFileSync(langFilePath, "utf-8");
      menuTranslations = JSON.parse(content2);
